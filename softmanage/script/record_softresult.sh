...
print_usage_and_exit () {
  cat <<-USAGE 1>&2
Usage   : ${0##*/} -l<soft ledger> -r<record file> <result>
Options :

update <record> with info of <soft ledger> and <result>.
<result> should be the format as below
  <the name of soft> <the result(OK/NG)> <the NG hosts>
...
if [ "${result}" != 'OK' -a "${result}" != 'NG' ]; then
  echo "INFO:${0##*/}: no result has been recorded for <${name}>. no host?" 1>&2
  exit 0
fi
...
exp=$(printf '. |= .+[{"date":"%s","result":"%s","ver":"%s","hosts":%s}]' \
