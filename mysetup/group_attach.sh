...
Attach groups to accounts on <inventory>.

-u: Specify the user name (default: <$(whoami)> = the user name who executes this).
-g: Specifiy the group list (defaul: <$(id -nG | tr ' ' ',')> = the groups to which this user belongs to).
-aj: Specify the json on which an array of user name and its type is defined. This is prioritized to -u and -g options. 
-tj: Specify the json on which an array of type name and its group is defined. This is prioritized to -u and -g options. 
